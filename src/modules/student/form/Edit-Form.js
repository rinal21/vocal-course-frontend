...
export default class studentEdit extends Component {
  ...
  fetchDatas = () => {
    ...
    fetch('localhost:8000/api/student/' + studentId)
      ...
      .then((json) => {
        json.map((data, index) => {
          this.setState({
            ...
            imgPreviewUrl: 'localhost:8000/signature/'+data.signature_img_url
          })
        })
  ...
  onSubmit(e) {
    ...
    console.log(obj)
    axios.patch('localhost:8000/api/student/'+this.props.studentId, obj)
        .then(res => console.log(res.data))
        ...
        validationSchema={StudentSchema}
            onSubmit={values => {
              if (this.props.status == 4 || this.props.status == 2) {
                ...
                console.log(obj)
                axios.patch('localhost:8000/api/student_status/' + this.props.studentId, obj)
                  .then(res => console.log(res.data))
                  .then(() => this.setState({ redirect: true }));
              }
            }
